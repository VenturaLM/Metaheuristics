...
def isEliteEqual(elite):
    #   TODO: NO FUNCIONA BIEN.
    for i in range(1, len(elite) - 1):
        if elite[0][0] == elite[i][0]:
            elite.pop(i)
    #########################
...
def main():
    ...
    maxGenerations = 10  # Number of generations/iterations [100 - 300].
    k = 3  # Tournament selector size.
    cProb = 0.7  # Cross probability.
    # Mutation probability. (In order to avoid randomness -->  [mProb <= 0.2] ).
    mProb = 0.1
