...
def applyGeneticOperator(population, mProb, largest_record_length):
...
def main():
    if len(sys.argv) == 2:
        ...
        population = []
        elite_threshold = 5
        dictionary = {'Elite': [], 'Profit': []}

        start_time = time.time()
        for constraint in nConstraints:
            s = evaluateGraph(
                records_df, constraint)
            ...
            population.append([s, len(s)/len(records_df['Event'])])
        ...
        while iterations <= maxGenerations:
            nSolutions = applyGeneticOperator(
                population, mProb, largest_record_length)
        ...
        elite_df = pd.DataFrame(dictionary, columns=['Elite', 'Profit'])
        print(elite_df)
